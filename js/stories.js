...
async function feedStoryfromFormToAddStory(evt){
  evt.preventDefault();
  console.debug("feedStoryfromFormToAddStory", evt);
  const story = await storyList.addStory(currentUser, {title: evt.target[1].value, author: evt.target[0].value, url: evt.target[2].value});
  $submitForm.hide();
  const $story = generateStoryMarkup(story);
  $allStoriesList.prepend($story);
}
$submitForm.on("submit", feedStoryfromFormToAddStory)
